...
class ThumbnailType extends React.Component {
	...
	handleOnChange = (id, value) => {
		...
		if (thumbnailTypeId == "new") {
			...
			if (set(thumbnailType, id, value)) {
				dispatch(updateNewThumbnailType(thumbnailType));
			}
		} else {
			thumbnailType = Object.assign(
				{},
				thumbnailTypes.filter(thumbnailType => {
	...
	handleOnAddNewThumbnailType = e => {
		...
		dispatch(
			...
			if (postedThumbnailType) {
				...
				dispatch(push("/thumbnail-type/" + postedThumbnailType.id + "/"));
	...
	handleOnDeleteThumbnailType = e => {
		...
		let thumbnailType = Object.assign(
			{},
			thumbnailTypes.filter(thumbnailType => {
				return thumbnailType.id == thumbnailTypeId;
			})[0]
		...
		dispatch(deleteThumbnailType(thumbnailType, accessToken)).then(success => {
			if (success) {
				dispatch(
					addNotification({
						title: "Deleted",
