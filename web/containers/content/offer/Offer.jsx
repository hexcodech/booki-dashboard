...
import { fetchBooksIfNeeded } from "core/actions/book";

import { addNotification } from "core/actions/notification";

import { selectInput, textAreaInput } from "web/utilities/input-types";
...
class Offer extends React.Component {
	...
	handleOnAddNewOffer(e) {
		...
		dispatch(postOffer(offer, accessToken)).then(postedOffer => {
			if (postedOffer) {
				dispatch(
					addNotification({
						title: "Created",
				...
				dispatch(push("/offer/" + postedOffer.id + "/"));
	...
	handleOnDeleteOffer(e) {
		...
		dispatch(deleteOffer(offer, accessToken)).then(success => {
			if (success) {
				dispatch(
					addNotification({
						title: "Deleted",
